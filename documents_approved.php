...
<link rel="shortcut icon" type="image/png" href="img/seal_icon.png">
    ...
    <title>Approved Documents | Barangay 837</title>
...
<div class="wrap-border">
    <br>
    <center>
      <h2 class="heading font-x2" style="font-size:40px;">All Approved Documents</h2>
    </center>
...
$sql = "SELECT * from doc_req INNER JOIN documents ON doc_req.doc_id = documents.doc_id WHERE doc_req.status='Approved'";
...
while ($row = mysqli_fetch_array($result)) { // fetch data from the mysqli_result object
    ...
    echo '<td class="std-cont"><a class="btn" href="view_document.php?doc_id=' . $row['doc_id'] . '&doc_type=' . $row['doc_type'] . '">View</a>&nbsp;&nbsp;';
    echo '<a class="btn" href="download_file.php?doc_id=' . $row['doc_id'] . '&doc_type=' . $row['doc_type'] . '">Download</a>';
    echo "</tr>";
}
?>
