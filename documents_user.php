...
<link rel="shortcut icon" type="image/png" href="img/seal_icon.png">
    ...
    <title>Documents | Barangay 837</title>
...
if(!$_SESSION['acc_privilege'] == 'User'){
...
<div class="wrap-border">
    <br>
    <center>
      <h2 class="heading font-x2" style="font-size:40px;">All Documents Requested</h2>
    </center>
...
$sql = "SELECT * from doc_req INNER JOIN documents ON doc_req.doc_id = documents.doc_id WHERE doc_req.acc_id='$user_id'";
...
while ($row = mysqli_fetch_array($result)) { 
    ...
    if($row['status'] == 'Pending') {
        ...
        echo '<td class="std-cont"><a class="btn" href="view_document.php?doc_id=' . $row['doc_id'] . '&doc_type=' . $row['doc_type'] . '">View</a></td>';
      } else if($row['status'] == 'Denied') {
        echo '<td class="std-cont"><a class= "btn" href="edit_document.php?doc_id='. $row['doc_id'] .'&doc_type=' . $row['doc_type'] . '">Edit</a>&nbsp;&nbsp;';
        echo '<a class="btn" href="view_document.php?doc_id=' . $row['doc_id'] . '&doc_type=' . $row['doc_type'] . '">View</a></td>';
    } else {
        echo '<td class="std-cont"><td></td>';
    }
    echo "</tr>";
}
?>
